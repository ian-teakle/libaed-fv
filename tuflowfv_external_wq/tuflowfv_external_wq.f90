...
TYPE :: fvwq
    ...
    REAL(wqrk),POINTER,DIMENSION(:) :: longwave             ! Longwave In (NC2)
    ...
    REAL(wqrk),POINTER,DIMENSION(:) :: humidity             ! Humidity (NC2)
...
!   WQFileName = 'aed.nml'
  ...
  wqmod = 'AED2'
  CALL init_aed_models(WQFileNum,WQFileName,Nwqvars,Nwqben,Nwqdiags,varnames,bennames,diagnames)
...
! BEGIN
  ...
  CALL init_var_aed_models(wq%nc3,wq%cc,wq%diag,wq%nwq,wq%nben, &
